...
export default function CircularSection({ 
  ...
  const handleStartWorkflow = async (item: any) => {
    try {
      const data = await workflowApi.startWorkflow(item.in_id);
      if (data.success) {
        Swal.fire({ icon: 'success', text: 'เริ่มกระบวนการเวียนหนังสือสำเร็จ', timer: 1500, showConfirmButton: false });
        onReload();
      }
    } catch (err: any) {
      Swal.fire('ผิดพลาด', err.response?.data?.message || err.message || 'ไม่สามารถดำเนินการได้', 'error');
    }
  };
