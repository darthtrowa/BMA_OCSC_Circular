...
import { BASE_URL, publicApi } from "../../api/apiService";
	...
	if (text.startsWith("http"))
		return (
			<a
				...
				target="_blank"
				rel="noreferrer"
				className="text-sky-600 hover:text-sky-800 underline decoration-sky-600/30 underline-offset-2 ml-2 text-sm inline-flex items-center"
			>
				<i className="bx bx-link-external mr-1"></i>ข้อมูลหนังสือเวียน
			</a>
		);
	return (
		<a
			onClick={(e) => e.stopPropagation()}
			href={`${BASE_URL}/uploads/${text}`}
			target="_blank"
			rel="noreferrer"
			className="text-rose-600 hover:text-rose-800 underline decoration-rose-600/30 underline-offset-2 ml-2 text-sm inline-flex items-center"
		>
			<i className="bx bxs-file-pdf mr-1"></i>หนังสือเวียนต้นฉบับ
		</a>
	);
}
...
function formatMati(
	...
	const d = obj[dateKey] as string | undefined;
			...
			<table className="w-full text-sm text-left border-collapse">
				<tbody className="divide-y divide-slate-100">
					<tr className="hover:bg-slate-50 transition">
						<td className="py-2.5 pr-4 font-semibold text-emerald-700 w-48 whitespace-nowrap">
							มติคณะทำงาน
						</td>
					...
					<tr className="hover:bg-slate-50 transition">
						<td className="py-2.5 pr-4 font-semibold text-emerald-700 whitespace-nowrap">
							มติ ก.ก
						</td>
						<td className="py-2.5 text-slate-700">
							{mkkText || mkkLink ? (
					...
					<tr className="hover:bg-slate-50 transition">
						...
						<td className="py-2.5 text-slate-700">
							{item.agency && item.agency.length > 0
								? item.agency.map((a) => <div key={a.ag_id}>{a.ag_name}</div>)
								: "-"}
						</td>
					</tr>
					<tr className="hover:bg-slate-50 transition">
						<td className="py-2.5 pr-4 font-semibold text-emerald-700 whitespace-nowrap align-top">
							รายละเอียดของหนังสือเวียน
						</td>
						<td
							className="py-2.5 text-slate-700"
							style={{ whiteSpace: "pre-wrap" }}
						>
							{item.in_circular_detail || "-"}
						</td>
					</tr>
					<tr className="hover:bg-slate-50 transition">
						<td className="py-2.5 pr-4 font-semibold text-emerald-700 whitespace-nowrap align-top">
							การพิจารณาจากส่วนราชการ
						</td>
						<td
							className="py-2.5 text-slate-700"
							style={{ whiteSpace: "pre-wrap" }}
							...
							{(item.categories || []).length > 0 ? (
								<div className="flex flex-wrap gap-1">
									{(item.categories || []).map((c) => (
										<span key={c.cat_id} className="inline-block">
											{c.cat_ref && c.cat_ref !== "-" ? (
												<a
													href={c.cat_ref}
													target="_blank"
													rel="noreferrer"
													className="text-emerald-600 hover:text-emerald-800 underline decoration-emerald-600/30 underline-offset-2"
												>
													{c.cat_name}
												</a>
											) : (
						...
						<td className="py-2.5 text-slate-700">
							{(item.references_info || []).length === 0 ? (
								...
								<div className="space-y-2">
									{(item.references_info || []).map((r) => (
										<button
											type="button"
											key={r.in_id}
											className={`w-full text-left p-3 rounded-xl border border-slate-100 bg-slate-50 ${r.in_id ? "cursor-pointer hover:bg-emerald-50 hover:border-emerald-100 transition" : ""}`}
											onClick={() => r.in_id && onRefClick(r.in_id)}
											disabled={!r.in_id}
										>
											<div className="font-semibold text-sky-700 text-sm mb-1">
												<span className="underline decoration-sky-700/30 underline-offset-2">
													เลขที่หนังสือ {r.in_num_date}{" "}
													{r.in_doc_date ? `ลงวันที่ ${r.in_doc_date}` : ""}
												</span>
											</div>
							...
							{parseAttachments(item.in_attachment_link).length > 0 ? (
								<div className="flex flex-col gap-1">
									{parseAttachments(item.in_attachment_link).map((att, idx) => (
										<a
											key={att}
											href={`${BASE_URL}/uploads/${att}`}
											target="_blank"
											rel="noreferrer"
											className="text-rose-600 hover:text-rose-800 underline decoration-rose-600/30 underline-offset-2 text-sm inline-flex items-center w-fit"
										>
											<i className="bx bxs-file-pdf mr-1"></i>เอกสารแนบท้าย
											{parseAttachments(item.in_attachment_link).length > 1
												? ` (${idx + 1})`
												: ""}
										</a>
									))}
		...
		<div className="mt-6">
			...
			<div className="grid grid-cols-1 gap-5">
				{paged.map((item) => {
					...
					if (resId === 2)
						resColor = "#10b981"; // นำมาใช้
					else if (resId === 4)
						resColor = "#f59e0b"; // นำมาปรับใช้
					else if (resId === 5)
						resColor = "#ef4444"; // ไม่ใช้
					else if (resId === 12)
						resColor = "#f97316"; // รอผล
					else if (resId === 11) resColor = "#64748b"; // ตกหล่น

					return (
						<div key={item.in_id} className="col-span-1">
							{/* biome-ignore lint/a11y/useSemanticElements: card container has nested links and buttons, cannot be a semantic button */}
							<div
								...
								<div className="p-5 md:p-6">
									<div className="flex justify-between items-start mb-4 gap-4">
										<div className="flex-1 min-w-0">
											...
											<div className="flex flex-wrap items-center gap-3 mb-2">
												...
												<div className="flex flex-wrap gap-2">
													{item.in_original_link &&
														item.in_original_link !== "-" && (
															<a
																onClick={(e) => e.stopPropagation()}
																href={
																	item.in_original_link.startsWith("http")
																		? item.in_original_link
																		: `${BASE_URL}/uploads/${item.in_original_link}`
																}
																target="_blank"
																rel="noreferrer"
															...
															>
																<i
																	className={
																		item.in_original_link.startsWith("http")
																			? "bx bx-link-external text-xs text-rose-500"
																			: "bx bxs-file-pdf text-xs text-rose-500"
																	}
																></i>
													...
													{item.in_qr_link && item.in_qr_link !== "-" && (
														<a
															onClick={(e) => e.stopPropagation()}
															href={
																item.in_qr_link.startsWith("http")
																	? item.in_qr_link
																	: `${BASE_URL}/uploads/${item.in_qr_link}`
															}
															target="_blank"
															rel="noreferrer"
											...
											<p
												className={`text-slate-700 mt-2 mb-3 text-base font-semibold ${!isOpen ? "line-clamp-2" : ""}`}
											>
												{item.in_detail}
											</p>
											...
											<div className="flex flex-col gap-1 text-slate-500 text-sm">
												{item.agency && item.agency.length > 0 ? (
													item.agency.map((a) => (
														<span key={a.ag_id} className="flex items-center">
										...
										<span
											...
											style={{
												backgroundColor: `${resColor}15`,
												color: resColor,
												borderColor: `${resColor}30`,
											}}
										>
											{resVal}
										</span>
									...
									<div className="flex flex-wrap gap-2 mb-4">
										...
										{(item.categories || []).map((c) => (
											<span
												key={c.cat_id}
												className="bg-sky-50 text-sky-600 border border-sky-100 py-1.5 px-3 rounded-full text-xs font-medium"
											>
												{c.cat_name}
											</span>
										))}
									...
									<div className="pt-4 border-t border-slate-100 flex justify-between items-center mt-2">
										<div className="text-slate-400 text-xs flex gap-3">
											<span className="flex items-center">
												...
												อัปเดตล่าสุด:{" "}
												{moment(item.updated_at || item.created_at)
													.locale("th")
													.add(543, "year")
													.format("DD MMM YYYY")}
											</span>
										...
										<div className="text-emerald-600 font-semibold text-sm flex items-center group-hover:text-emerald-700">
											...
											<i
												className={`bx ${isOpen ? "bx-chevron-up" : "bx-chevron-down"} ml-1 text-lg`}
											></i>
										</div>
									</div>
			...
			{totalPages > 1 && (
				<div className="mt-10 flex flex-col items-center gap-3">
					...
					<small className="text-slate-400 font-medium">
						...
						{Math.min(page * perPage, filtered.length)} จาก {filtered.length}{" "}
						รายการ
					</small>
				</div>
			)}
			...
			{activeRefData && (
				<div
					className="fixed inset-0 z-100 flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm animate__animated animate__fadeIn animate__faster"
					onClick={() => setActiveRefData(null)}
					role="dialog"
					aria-modal="true"
					tabIndex={-1}
					onKeyDown={(e) => {
						if (e.key === "Escape") setActiveRefData(null);
					}}
				>
					{/* biome-ignore lint/a11y/noStaticElementInteractions: stop click bubbling to backdrop overlay */}
					<div
						className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate__animated animate__zoomIn animate__faster"
						onClick={(e) => e.stopPropagation()}
						role="presentation"
					>
						...
						<div className="p-6 overflow-y-auto custom-scrollbar flex-1">
							<h5 className="font-bold mb-1 text-slate-800">
								{activeRefData.in_num_date}
							</h5>
							{activeRefData.in_doc_date && (
								<div className="text-xs text-slate-500 mb-2">
									<i className="bx bx-calendar-event mr-1"></i>ลงวันที่{" "}
									{activeRefData.in_doc_date}
								</div>
							)}
