            ...
            {loading ? (
              ...
              users.map((u: any, i: number) => (
                <tr key={u.a_id} className="hover:bg-slate-50 border-b border-slate-100 transition last:border-0">
                  ...
                  <td className="px-6 py-4"><span className="font-semibold text-slate-800">{u.a_name}</span></td>
                  <td className="px-6 py-4 text-slate-600">{u.a_agency || <span className="text-slate-400">-</span>}</td>
                  <td className="px-6 py-4">
                    <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded font-mono text-xs">{u.a_username}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 text-xs font-bold rounded-full ${
                      u.a_permiss === 'superadmin' ? 'bg-rose-100 text-rose-700' : 
