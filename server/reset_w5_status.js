...
const DOC_TARGET = 'ว5/2569';
const DOC_SOURCE = 'ว4/2569';
  ...
  try {
    ...
    const srcRes = await pool.query(
      `SELECT in_id, in_detail, in_num_date, in_workflow_status, in_current_owner_id, in_flow_state 
       FROM c_information 
       WHERE in_detail LIKE $1 OR in_num_date LIKE $1 LIMIT 1`,
      [`%${DOC_SOURCE}%`]
    );
    
    if (srcRes.rows.length === 0) {
      console.error(`[ERROR] Source document (${DOC_SOURCE}) not found!`);
      return;
    }
    
    const srcDoc = srcRes.rows[0];
    console.log(`\n[Source] Found ${DOC_SOURCE}:`);
    ...
    if (tgtRes.rows.length === 0) {
      console.error(`[ERROR] Target document (${DOC_TARGET}) not found!`);
      return;
    }
    
    const tgtDoc = tgtRes.rows[0];
    console.log(`\n[Target] Found ${DOC_TARGET} (Current state):`);
    ...
    console.log(`\n[Updating] Copying status from ${DOC_SOURCE} to ${DOC_TARGET}...`);
    ...
    console.log(`[SUCCESS] Reset status for ${DOC_TARGET} completed!`);
