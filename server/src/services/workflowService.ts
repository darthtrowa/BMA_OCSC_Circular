...
export class WorkflowService {
  ...
  static async simulateNextAssignees(
    taskState: { in_workflow_status: string; in_flow_state: string | null; agencies: number[] },
    ...
    const baseUserRes = await db.query(`SELECT a_id, a_role, a_agency_id FROM admin WHERE a_id = $1`, [currentUserId]);
    ...
    if (!resolvedDelegationId && requiredRole && baseUser.a_role !== requiredRole) {
    ...
    if (resolvedDelegationId) {
      const delRes = await db.query("SELECT assigner_id, delegated_role, assigner_ag_id FROM c_workflow_delegations WHERE delegation_id = $1", [resolvedDelegationId]);
      if (delRes.rows.length > 0) {
        const delRow = delRes.rows[0];
        if (delRow.assigner_id) {
          effectiveUserId = delRow.assigner_id;
    ...
    const getUserAgencyInfo = (u: any) => {
      const ag = allUsers.find(au => Number(au.a_id) === Number(u.a_id));
      return {
        ag_type: u.ag_type || (ag ? ag.ag_type : null),
        parent_ag_id: u.parent_ag_id || (ag ? ag.parent_ag_id : null)
      };
    };
  ...
  static async simulateNextAction(
    task: any,
    activeUserId: number,
    action: 'FORWARD' | 'REJECT' | 'CLOSE' | 'PARALLEL_ASSIGN' | 'PARALLEL_DELEGATE' | 'PARALLEL_SUBMIT',
    payload: {
      targetUserId?: number;
      comments?: string;
      delegationId?: number;
    ...
    if (!resolvedDelegationId && requiredRole && activeUser.a_role !== requiredRole) {
      const activeDelRes = await db.query(
        `SELECT delegation_id FROM c_workflow_delegations 
         WHERE assignee_id = $1 AND delegated_role = $2 AND is_active = true LIMIT 1`,
    ...
    const { role: effectiveRole } = await this.getSimulatedEffectiveUser(task, activeUser, resolvedDelegationId);
    ...
    if (action === 'FORWARD') {
      ...
      if (!targetUserId) throw new Error("Target user ID is required for forward");
      ...
      let newStatus = this.getNextStatus(effectiveRole, targetUser.a_role);
      ...
      const targetUserId = payload.targetUserId;
      if (!targetUserId) throw new Error("Target user ID is required for reject");
      ...
      if (taskAgencies.length === 0) throw new Error("No agencies selected for parallel distribution");

      const parallelTracks: any[] = [];
      for (let idx = 0; idx < taskAgencies.length; idx++) {
        const agId = taskAgencies[idx];
        
        const agRes = await db.query("SELECT ag_name FROM c_agency WHERE ag_id = $1", [agId]);
        const agName = agRes.rows.length > 0 ? agRes.rows[0].ag_name : `หน่วยงาน ${agId}`;
        ...
        const ownerPosition = director ? director.a_position || director.a_role : activeUser.a_position;
      ...
      const targetUserId = payload.targetUserId;
      if (!paId || !targetUserId) throw new Error("paId and targetUserId are required for parallel delegation");

      const subRes = await db.query(
        `SELECT a_id, a_name, a_position, a_role FROM admin WHERE a_id = $1`, 
        [targetUserId]
      );
      if (subRes.rows.length === 0) throw new Error("Subordinate user not found");
      const subUser = subRes.rows[0];

      const updatedTracks = (task.parallel_assignments || []).map((pa: any) => {
        if (pa.pa_id === paId) {
          return {
            ...pa,
      ...
      if (!paId || !resultsId) throw new Error("paId and resultsId are required for parallel submit");

      const resTextRes = await db.query("SELECT results_detail FROM c_results WHERE results_id = $1", [resultsId]);
      const resultsText = resTextRes.rows.length > 0 ? resTextRes.rows[0].results_detail : 'พิจารณาแล้ว';

      const track = (task.parallel_assignments || []).find((pa: any) => pa.pa_id === paId);
      ...
      const terminalTracks = updatedTracks.filter((pa: any) => ['SUBMITTED', 'REJECTED'].includes(pa.pa_status)).length;

      let newStatus = 'PENDING_PARALLEL';
      let newCurrentOwnerId = task.in_current_owner_id;

      if (totalTracks === terminalTracks) {
        const hrRes = await db.query("SELECT a_id, a_name, a_position, a_role FROM admin WHERE a_role = 'HR_DIRECTOR' LIMIT 1");
        ...
        newHistoryEntries.push({
          ...
          to_user_position: hrDir ? hrDir.a_position || hrDir.a_role : 'HR_DIRECTOR',
   ...
   */
  private static async getSimulatedEffectiveUser(task: any, user: any, delegationId?: number) {
